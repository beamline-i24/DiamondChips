...
from Chip_StartUp_v7_ADE import scrape_parameter_file
from Chip_StartUp_v7_ADE import get_format
from Chip_Manager_v7_ADE import moveto
...
def start_sacla():
	...
	lg.warning('%s Unknown Map Type, map_type = %s'%(name,map_type))
        print 'Unknown map type'
    #SACLA4 commented out for for ADE
    """
...
def get_prog_num(chip_type, map_type):
    ...
    if chip_type in ['0', '1', '2', '5']:
	    ...
	    lg.info('%s\t:Map Type = Photocage / Mapping Lite'%name)
    	    print 'Map Type is Photocage /Mapping lite'
            return 14
        else:
...
def main(location='i24'):
    ...
    #caput(pv.me14e_pmac_str, '!x0y0z0')
    # ADE
    caput(pv.me14e_pmac_str, '!x-2y-2z0')
    # ADE
    sleep(3.5)
