...
def get_alphanumeric(chip_type):
    lg.info('CHIP_STARTUP\tget_alphanumeric - start')
    lg.warning('CHIP_STARTUP\tget_alphanumeric - warnline')
    try:
        lg.error('errr')
    except:
        print 'oops'
    try:
        lg.fatal('oh no this is fatal')
    except:
        print 'oops'
    try:
        lg.warning('loggy 1 %s', exc_info=1)
        lg.log(0, 'level test at 0')
        lg.log(30, 'level test at 30')
        lg.log(50, 'level test at 50')
        print panda
    except:
        lg.warning("loggy 2 %s", "what is gggggg", exc_info=1)
        print 'oopiiiiis'
...
def write_headers(location, suffix_list):
    lg.info('CHIP_STARTUP\twrite_headers:trying')
    if location == 'i24':  
        chip_name, visit, sub_dir, n_exposures, chip_type, map_type, exptime, dcdetdist = scrape_parameter_file('i24')
        a_directory = '/dls_sw/i24/scripts/fastchips/'
    elif location == 'SACLA':
