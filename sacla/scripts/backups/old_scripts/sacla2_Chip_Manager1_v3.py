...
import sacla2_Chip_StartUp1_v3 as su
import sacla2_Chip_Mapping1_v3 as mapping
...
def initialise():
    ...
    caput(pv.me14e_filter + '.HLM', 25)
    caput(pv.me14e_filter + '.LLM', -25)
...
def moveto(place):
    ...
    if place == 'zero':
        ...
        caput(pv.me14e_filter, 20.0)  

    elif place == 'lightout':
        caput(pv.me14e_filter, -18.0)
...
def cs_maker():
    ...
    if chip_type == 0:
        ...
        x1factor = (f1_x / 17.16)  * scale
        y1factor = (f1_y / f1_x)   * scale
        z1factor = (f1_z / f1_x)   * scale
        x2factor = (f2_x / f2_y)   * scale
        y2factor = (f2_y / 26.49)  * scale
        z2factor = (f2_z / f2_y)   * scale
        z3factor = scale
	...
	print 'Oxford Chip Shift Origin'
        ...
        sleep(0.5)
        caput(pv.me14e_pmac_str, '!X-3.9Y0.765Z0')
