...
import os, re, sys
import numpy as np
from pathli import pathli
from matplotlib import pyplot as plt

def fiducials(chip_type='classic'):
    if chip_type == 'Classic'
        ...
        print 'Unrecognized chip_type, returning empty fid_list & corners_list'
        fid_list = [] 
        corners_list = [] 
        return fid_list, corners_list   
...
def normal_dicts(chip_type):
    if chip_type == 'classic':
            ...
            for r in range(9):
                ...
                for wc in range(12):
                    ...
                    for wr in range(12):
                        ...
                        #print i,
                        i += 1
    elif chip_type == 'test1':
        road_list = ['A','B','C','D']
        cros_list = ['1','2','3','4']
        wind_list = ['a','b','c']
        #alphanumeric
        windlong = pathli(wind_list, len(wind_list), 'typewriter')
        print windlong
        ...
        print 'Unrecognized chip_type, returning empty dictionaries'
        ordr_dict = {}
        addr_dict = {}
        return addr_dict, ordr_dict

def get_xy(chip_type='classic', addr):
    if chip_type == 'classic'
        w2w = 0.125
        b2b_horz = 0.825
        b2b_vert = 1.125
        ...
        print 'Unrecognized chip_type'
        print 'Returning random distribution'
...
def main():
    ...
    fid_list, corners_list = fiducials(chip_type)

    x_list, y_list, z_list = [], [], []
    for i in sorted(normal_ordr_dict.keys()):
        addr = normal_ordr_dict[i]
        x, y = get_xy(chip_type, addr)
