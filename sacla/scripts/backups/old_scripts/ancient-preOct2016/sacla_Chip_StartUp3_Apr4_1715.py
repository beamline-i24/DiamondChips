        ...
        for R in range(9):
            for c in range(12):
                for r in range(12):
                    if (C % 2 == 0):
                        if (c % 2 == 0):
                            addr = road_list[R] + cros_list[R] + '_' + wind_list[c] + wind_list[r]
                        else:
...
def shotLister_W_SnakeRows(chipname):
    f = open(path + '/' + fid, 'r')
...
def shotLister_E_SnakeCols_reverse(chipname):
    print 'Writing Shot'
    f = open(path + '/' + fid, 'r')
    header = f.readlines()[:11]
    f.close()
...
def shotLister_L2RWs_TypewriterRows(chipname):
    f = open(fid, 'r')
