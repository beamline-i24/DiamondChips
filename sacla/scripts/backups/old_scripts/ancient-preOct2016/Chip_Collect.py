...
def setup_pilatus(chipname, visit_id, filepath, total_numb_imgs, exptime, dcdetdist):
    ...
    caput(pv.pilat_beamx, 1266.7)
    caput(pv.pilat_beamy, 1324.6)
...
def setup_beamline(hutch, dcdetdist):
    if hutch == 'ready for data collection':
        ...
        sleep(1)
    if hutch == 'chipsnapshot':
        caput(pv.det_z, dcdetdist)
        caput(pv.blighty_pos, 'OAV2')
        caput(pv.me14e_stage_x, 9.96)
        caput(pv.me14e_stage_y, 8.98)
        sleep(1)
...
def main():
    ...
    copyfile('/dls_sw/i24/scripts/fastchips/parameter_files/setdcparams.txt', \
        ...
        #################################
    setup_beamline('chipsnapshot', dcdetdist)
    sleep(2)
    oav2_image = urllib.urlopen('http://bl24i-di-serv-01.diamond.ac.uk:8081/oav2.mjpg.jpg')
    g = open(path + filepath + '-' + chipname + '-' + str(run_num) + '.jpg', 'w')
    g.write(oav2_image.read())
    g.close()
        #################################

    setup_beamline('ready for data collection', dcdetdist)
    sleep(5) # this is just to wait to make sure the detector is in position
    print '--------------'
    ...
    for addr in path_type:
        ...
        sleep(0.1)
        print 3*'--------------------------------\n'
        caput(pv.me14e_pmac_str, '&1b20r')                       # Run motion program 10
        print '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
