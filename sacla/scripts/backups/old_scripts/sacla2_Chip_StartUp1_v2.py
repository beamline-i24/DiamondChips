...
def make_path_dict():
    ...
    path99 = ['A1_aa','B1_aa','C1_aa','D1_aa','E1_aa','F1_aa','G1_aa','H1_aa','I1_aa',\
    ...
    classic_list = [path11, path33, path55, path77, path99]

    #path_dict[path_key] = [xstart, ystart, xblocks, yblocks, [path]]
    path_dict = {}
    for path in classic_list:
	# Check that the list has an equal humber of blocks with the same starting letter
        k =  [i[0] for i in path]
        occurence = [k.count(x) for x in k]
        if len(set(occurence)) == 1:
            xblocks = int(occurence[0])
        else:
            print 'Failed occurence test'
	    print 'unable to define xblocks'
	    return 0
        yblocks = int(len(set([x[0] for x in path])))
        path_key = str(xblocks) + str(yblocks) + '_classic'
        xstart, ystart = get_xy(path[0])
	    ...
	    print 'coltype not aa or la'
        path_dict[path_key]=[string_xstart, string_ystart, xblocks, yblocks, coltype, path]

    #path_dict[path_key] = [xstart, ystart, xblocks, yblocks, [path]]
    magis_list = []
    for path in magis_list:
	print 'no magis list yet'
        path_dict['88_magis'] = []

    #path_dict[path_key] = [xstart, ystart, xblocks, yblocks, [path]]
    hll_list = []
    for path in hll_list:
	print 'no hll list yet'
        path_dict['33_hll'] = []
...
def scrape_dcparameters():
    ...
    for line in f:
        ...
        if 'chipname' in entry[0].lower():
            ...
            proteinname = entry[1]
        elif 'num_of_shots' in entry[0].lower():
	    num_of_shots = entry[1]
	    ...
	    map_type = entry[1]
            ...
            path_key = entry[1]
    return chipname, visit_id, proteinname, num_of_shots, chip_type, map_type,  path_key

def alphanumeric(chipname):
    list_of_lines = []
...
def write_file(suffix='.addr', order='alphanumeric'):
    ...
    if order == 'alphanumeric':
        for line in alphanumeric(chipname):
            g.write(line)
    elif order == 'shotorder':
        for line in shotorder(chipname):
            g.write(line)
    else:
	print '?'
    g.close() 
...
def main():
    ...
    write_file('.spec', 'shotorder')
    write_file('.shot', 'shotorder')
