...
def scrape_parameter_file(location=None):
    ...
    for line in f:
        ...
        if 'chip_name' in entry[0].lower():
            ...
            n_exposures = entry[1]
        elif 'n_pump_exposures' in entry[0].lower():
            n_pump_exposures = entry[1]
    ...
    if location == 'i24':
        ...
        for line in f:
            ...
            if 'dcdetdist' in entry[0].lower():
                dcdetdist = entry[1]
        return chip_name, visit, sub_dir, n_exposures, n_pump_exposures, chip_type, map_type, exptime, dcdetdist
   
    else:
        return chip_name, sub_dir, n_exposures, n_pump_exposures, chip_type, map_type
...
def get_format(chip_type):
	...
	#Heidelberg 4 75um 2x2x120x60
        #w2w = 0.075
        w2w = 0.180
        b2b_horz = 1.285
        b2b_vert = 0.785
        #chip_format = [2, 2, 120, 60] #ORGINAL
        #chip_format = [2, 2, 75, 37] #For 120 micron
	lg.warning('RUNNING IN SINGLE WINDOW MODE')
        #chip_format = [1, 1, 50, 25] #For 180 micron
        chip_format = [2, 2, 50, 25]
    elif chip_type == '8':
	#Heidelberg 6 75um 3,2,80,60
        #w2w = 0.075
        w2w = 0.180
        b2b_horz = 0.875
        b2b_vert = 1.085
        #chip_format = [3, 2, 80, 56]#ORIGINAL
        #chip_format = [3, 2, 50, 36]#For 120 micron
	lg.warning('RUNNING IN SINGLE WINDOW MODE')
        chip_format = [1, 1, 34, 24]
    else:
        lg.warning('%s Unknown chip_type, %s, in fiducials'%(name, chip_type))
...
def write_file(location='i24', suffix='.addr', order='alphanumeric'):
    ...
    lg.info('%s'%name)
    if location == 'i24':  
        chip_name, visit, sub_dir, n_exposures, n_pump_exposures, chip_type, map_type, exptime, dcdetdist = scrape_parameter_file('i24')
        a_directory = '/dls_sw/i24/scripts/fastchips/'
    elif location == 'SACLA':
        chip_name, sub_dir, n_exposures, n_pump_exposures, chip_type, map_type = scrape_parameter_file()
...
def run():
    ...
    print 'Run StartUp'
    lg.info('%s' %name)
    check_files('SACLA', ['.addr', '.shot'])
    print 'Checked files'
    lg.info('%s Checked Files' %name)
    write_headers('SACLA', ['.addr', '.shot'])
    ...
    check_files('SACLA', ['rando.spec'])
    write_headers('SACLA', ['rando.spec'])
