...
from sacla3_Chip_StartUp_v7 import *
...
def get_hamburg_order():
    ...
    lg.info('%s' %(name))
    blk_num = 3
    caps = ['A','B','C','C','B','A','A','B','C','C','B','A','A','B','C','C','B','A']
...
def convert_chip_to_hex(fid, chip_type):
	...
	lg.info('%s'%shot_order_list[-14:])
        ...
        for i in range(number_of_lines): 
            sublist = sorted_pres_list[i * windows_per_block : (i * windows_per_block)+windows_per_block]
            if i%2 == 0:
                right_list = sublist
            else:
                right_list = sublist[::-1] 
            hex_string = ("{0:0>%sX}" %hex_length).format(int("".join(str(x) for x in right_list),2))
            hex_string = hex_string + pad * '0'
            pvar = 5001+i
            line = 'P%s=$%s' %(pvar, hex_string)
            g.write(line+'\n')
            print ("{0:0>%sX}" %hex_length).format(int("".join(str(x) for x in right_list),2)) + 4*'0',
            print i, right_list, 
            print "".join(str(x) for x in right_list), 
            print line
	    lg.info('%s %s \n' %(name, ("{0:0>%sX}" %hex_length).format(int("".join(str(x) for x in right_list),2)) + 4*'0'))
	    lg.info('%s %s %s \n' %(name,i, right_list))
        ...
        for col in range(6): 
            if col %2 == 0:
                for line in range(159):
                    ...
                    if even_odd == 53:
                        even_odd = 0
                    hex_string = ("{0:0>7X}").format(int("".join(str(x) for x in bite),2))
                    pvar = 5001+i
                    writeline = 'P%s=$%s' %(pvar, hex_string)
                    g.write(writeline+'\n')
                    print line, "".join(str(x) for x in bite), 
...
def main():
    name = inspect.stack()[0][3]
    lg.info('%s' %name)
    chip_name, visit, sub_dir, n_exposures, n_pump_exposures, chip_type, map_type = scrape_parameter_file()
