    ...
    if location == 'i24':
        ...
        for line in f:
            ...
            if 'dcdetdist' in entry[0].lower():
                dcdetdist = entry[1]
        return chip_name, visit, sub_dir, n_exposures, chip_type, map_type, exptime, dcdetdist
...
def write_file(location='i24', suffix='.addr', order='alphanumeric'):
    ...
    lg.info('%s'%name)
    if location == 'i24':  
        chip_name, visit, sub_dir, n_exposures, chip_type, map_type, exptime, dcdetdist = scrape_parameter_file('i24')
