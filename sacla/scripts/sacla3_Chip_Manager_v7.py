...
lg.basicConfig(format='%(asctime)s %(levelname)s:   \t%(message)s',level=lg.DEBUG, filename='SACLA4v1.log')
##############################################
# MANAGER  MANAGER  MANAGER  MANAGER MANAGER # 
# This version last edited 19Mar2018 by RLO  #
# tweaks for SACLA5                          #
# added n_pump_exposures                     #  
##############################################
...
def initialise():
    ...
    sleep(0.1)
    print 'Clearing'
    lg.info('%s Clearing General Purpose PVs 1-100' %name)
...
def write_parameter_file():
    ...
    n_pump_exposures = caget(pv.me14e_gp98)  
    ...
    f.write('n_pump_exposures \t%s\n' %n_pump_exposures)
    f.write('chip_type \t%s\n' %chip_type)
    f.write('map_type \t%s\n' %map_type)
    f.close()

    lg.info('%s chip_name: %s' %(name, chip_name))
    lg.info('%s protein_name: %s' %(name, protein_name))
    lg.info('%s n_exposures: %s' %(name, n_exposures))
    lg.info('%s n_pump_exposures: %s' %(name, n_pump_exposures))
    ...
    print 'n_pump_exposures', n_pump_exposures
...
def load_lite_map():
    ...
    elif chip_type == 1 or chip_type == 3:
...
def load_full_map(location='SACLA'):
    name = inspect.stack()[0][3]
    if location == 'i24':
    	chip_name, visit, sub_dir, n_exposures, n_pump_exposures, chip_type, map_type = startup.scrape_parameter_file(location)
    else:
	chip_name, sub_dir, n_exposures, n_puimp_exposures, chip_type, map_type = startup.scrape_parameter_file(location)
    #fullmap_path = '/dls_sw/i24/scripts/fastchips/fullmaps/'
    fullmap_path = '/localhome/local/Documents/sacla/parameter_files/'
    ...
    lg.info('%s cp %s %s' %(name, fullmap_fid[:-4]+'full', fullmap_path+'currentchip.full'))
    print 10*'Done ', '\n'
...
def moveto(place):
    ...
    if place == 'zero':
        ...
        lg.info('%s moving to %s'%(name,place))
        caput(pv.me14e_pmac_str, '!x-0.09y-0.09z0')
        sleep(0.1)
        caput(pv.me14e_pmac_str, '#1hmz#2hmz#3hmz')

    elif place == 'offset2':
        lg.info('%s moving to %s'%(name,place))
        caput(pv.me14e_pmac_str, '!x0.09y0.09z0')
        sleep(0.1)
        caput(pv.me14e_pmac_str, '#1hmz#2hmz#3hmz')
    ...
    elif place == 'attenuatorin':
        print('attenuator in')
        lg.info('%s Attenuator In' %(name))
        lg.debug('%s nb need M608=100 M609 =150 somewhere'%name)
        ##### nb need M608=100 M609 =150 somewhere 
        caput(pv.me14e_pmac_str, 'M612=1 M611=1')

    elif place == 'attenuatorout':
        print('attenuator out')
        lg.info('%s Attenuator out' %(name))
        caput(pv.me14e_pmac_str, ' M612=0 M611=1')

    elif place == 'flipperin':
        print('flipper in')
        lg.info('%s Flipper In' %(name))
        lg.debug('%s nb need M708=100 M709 =150 somewhere'%name)
        ##### nb need M708=100 M709 =150 somewhere 
...
def scrape_mtr_directions():
    ...
    for line in f.readlines():
        if line.startswith('mtr1'):
            ...
            continue
    f.close()
    lg.info('%s mt1_dir %s mtr2_dir %s mtr3_dir %s'%(name,mtr1_dir, mtr2_dir, mtr3_dir))
...
def cs_maker():
    ...
    lg.warning('cs_maker has been changed for chip type 8')
    fiducial_dict[8] = [8.725, 8.725]
    print 'yo'
    print chip_type, fiducial_dict[chip_type]
    print 'yo'
    lg.info('%s chip type is %s with size %s'%(name, chip_type,fiducial_dict[chip_type]))
    ...
    #SACLA4
    scalex, scaley, scalez  = 10010.2, 9996.4, 10000.0
    ...
    lg.info('%s These should be 1. This is the sum of the squares of the factors divided by their scale'%(name))
