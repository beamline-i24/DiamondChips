...
from sacla3_Chip_Manager_v7 import moveto
...
# This version last edited 20Apr2018 by RLO  #
# tweaks for SACLA5                          #
# added n_pump_exposures, 2s and 3s photocage#  
##############################################
...
def get_chip_prog_values(chip_type, location, exptime=16, n_exposures=1, n_pump_exposures=0):
	...
	lg.info('%s This is a Custom Chip'%name)
    	...
    	x_step_size = caget(pv.me14e_gp8)
    	#y_step_size = x_step_size
        #change at sacla4. Check to a better PV afterwards
    ...
    {'X_NUM_STEPS':  [11, x_num_steps],
     ...
     'N_PUMP_EXPOSE':[40, n_pump_exposures]}
...
def start_i24():
    ...
    chip_name, visit, sub_dir, n_exposures, n_pump_exposures, chip_type, map_type, exptime, \
	...
	lg.info('%s\t:chip_format=%s'%(name,chip_format))
        ...
        print 'n_exposures n_pump_exposures', n_exposures, n_pump_exposures
...
def start_sacla():
    ...
    start_time = time.ctime()
    chip_name, sub_dir, n_exposures, n_pump_exposures, chip_type, map_type = scrape_parameter_file(location='SACLA')
    #### Hack for sacla3 to bismuth chip type for oxford inner
    if str(chip_type) =='3':
	    lg.debug('%s\t:Hack for SACLA3 bismuth for oxford inner'%name)
            chip_type = '1'
    ...
    elif map_type == '1' or map_type == '3' or map_type == '4':
        chip_format = get_format(chip_type)[2:4]
        block_count = 0
	...
	lg.warning('%s Not Set Up For Full Mapping'%name)
        print 'FIX ME. I am not set up for full mapping '
    else:
	lg.warning('%s Unknown Map Type, map_type = %s'%(name,map_type))
        print 'Unknown map type'
    ###make sure flipper light and attenuator are all out
    moveto('flipperout')
    sleep(1)
    moveto('attenuatorout')
    sleep(1)
    moveto('lightout')
    sleep(3)
    return start_time
...
def get_prog_num(chip_type, map_type):
    ...
    if chip_type in ['0', '1', '2', '5']:
	    ...
	    lg.info('%s\t:Map Type = 2s Photocage / Mapping Lite'%name)
    	    print 'Map Type is 2s Photocage /Mapping lite'
            return 14
        elif map_type == '4': 
	    lg.info('%s\t:Map Type = 3s Photocage / Mapping Lite'%name)
    	    print 'Map Type is 3s Photocage /Mapping lite'
            return 15
        else:
...
def main(location='i24'):
    ...
    if str(chip_type) =='3':
	    lg.debug('%s\tMain:Hack for SACLA3 Bismuth to Oxford Inner'%name)
            chip_type = '1'
    if location =='i24':
		chip_prog_dict = get_chip_prog_values(chip_type, location, exptime=exptime, n_exposures=n_exposures, n_pump_exposures=n_pump_exposures)
    else:
    	chip_prog_dict = get_chip_prog_values(chip_type, location, n_exposures=n_exposures, n_pump_exposures=n_pump_exposures)
