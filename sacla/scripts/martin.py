...
def upload_parameters(chipid):
    ...
    print
    print 'Setting Mapping Type to Lite'
    lg.info('%s Setting Mapping Type to Lite'%name)
    caput(pv.me14e_gp2, 1)
    print 10*'Done '
    lg.info('%s %s'%(name,10*'Done'))
...
def upload_full():
    ...
    lg.info('%s Opening %scurrentchip.full' %(name, fullmap_path))
...
def moveto(place):
    ...
    if place == 'zero':
        ...
        lg.debug('%s nb need M508=100 M509 =150 somewhere'%sname)
